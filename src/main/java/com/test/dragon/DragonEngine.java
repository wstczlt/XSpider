...
public class DragonEngine {
  ...
  public DragonEngine(Supplier<OkHttpClient> clientSupplier, ExecutorService pool,
      Supplier<List<Integer>> matchSupplier, Logger logger) {
    mClientSupplier = clientSupplier;
    mPool = pool;
    mLogger = logger;
    mMatchSupplier = matchSupplier;
    mDragonProcessor = new DragonProcessor(logger);
  }
